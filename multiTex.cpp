...
int multiTex::loadTextures(const char* texFile[]) {
  for(int i = 0; i < numTextures; i++) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, imgArray[i]->format->BytesPerPixel, imgArray[i]->w, imgArray[i]->h, 0, textureFormat(imgArray[i]->format, texFile[i]), GL_UNSIGNED_BYTE, imgArray[i]->pixels);
  }
  return 0;
}
